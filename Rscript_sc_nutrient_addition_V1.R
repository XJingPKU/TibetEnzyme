...
df$treat <- factor(df$treat, ordered = TRUE,
                   levels = c("control", "N25", "N50", "N100", "P50", "N100P50"))
...
# clean the data for N100 in block 5 for specific enzyme activity
df$value[df$variable %in% df$variable[grep("mbc", df$variable)]
         & df$treat == "N100" & df$block == "5"] <- NA

# t test
nenz <- levels(df$variable)
ntreat <- c("N25", "N50", "N100", "P50", "N100P50")
...
# store the results in ./data/data_processing/enz_dat_short_comm_NutA_V1.csv
